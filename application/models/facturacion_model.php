...
// 			$res = $this->db->query("
    ...
    $datosApi = $this->cfdi->obtenDatosCfdi33($_POST, $productos);
      ...
      echo "</pre>";exit;
			...
			if ($result['passes'])
			{
				...
				$pathDocs = $this->documentos_model->creaDirectorioDocsCliente($datosCadOrig['nombre'], $data['serie'], $data['folio']);
	...
	* @return array
	...
	public function getDataFactura($id_factura=null, $ivass=false, $sql = ''){
		...
		$res_q1 = $this->db->select("f.*,cl.email as email_cliente,em.email as email_empresa,f.id_empresa as empresa_id,em.rfc as empresa_rfc,em.nombre_fiscal as empresa_nombre_fiscal,em.logo")->from('facturacion as f')->join('empresas as em','f.id_empresa = em.id_empresa','left')->join('clientes as cl','cl.id_cliente = f.id_cliente','left')->where($sql)->order_by('serie asc, folio asc')->get();
